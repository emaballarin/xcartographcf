...
class FontnameParser:
    ...
    def get_name_token(name, tokens):
        ...
        while j:
            ...
            not_matched += (
                " " + j.groups()[0]
            )  # Blank prevents unwanted concatenation of unmatched substrings
    ...
    def parse_font_name(name):
        ...
        other += [
            ...
            "2m",
            "2p",
        ]  # MPlus names need to be preserved (sort array elements long to short, i.e. '1mn' first)
        ...
        if len(style) < 4:
            (style, weight_token_abbrevs) = FontnameParser.get_name_token(
                style, weight_abbrevs
            )
            (style, style_token_abbrevs) = FontnameParser.get_name_token(
                style, style_abbrevs
            )
        ...
        if "-" in style:
            ...
            return FontnameParser.parse_font_name(
                familyname + matches.group(1) + "-" + matches.group(2)
            )
    ...
    def __init__(self, filename):
        ...
        [
            ...
            self.other_token,
            self.rest,
        ] = FontnameParser.parse_font_name(filename)
    ...
    def fullname(self):
        ...
        if "Regular" in styles:
            if (
                not self.keep_regular_in_family  # User says: Regular is the normal font, so it is not mentioned
                or len(self.weight_token) > 0
    ...
    def psname(self):
        ...
        for c in (
            FontnameParser.concat(
                self.basename, self.rest, self.other_token, self.fontname_suff
            )
            + "-"
            + FontnameParser.concat(self.weight_token, self.style_token)
        ):
            if c in "[](){}<>/%":
                continue
    ...
    def preferred_family(self):
        ...
        if self.suppress_preferred_if_identical and len(self.weight_token) == 0:
            ...
            return ""
        return FontnameParser.concat(
            self.basename, self.rest, self.other_token, self.family_suff
        )
    ...
    def family(self):
        ...
        return FontnameParser.concat(
            self.basename, self.rest, other, self.family_suff, weight
        )
